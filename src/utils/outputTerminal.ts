...
async function logMessage(mess: string): Promise<void> {
...
async function messageRainbow(mess: string): Promise<void> {
...
// Format money
function moneyFormat(str: string): string {
    let fraction: string = str.slice(str.indexOf(".") + 1, str.length);
    let integer: string = str.slice(0, str.indexOf("."));
    let result: string[] = [];
